...
const int MAX_DEPTH					=	6;   //Depth of the octree
...
public:
	...
	inline COctree**GetChildren() { return m_pobChildren; }
	inline int		GetDepth() { return m_iLevel; }
	inline std::vector<double> GetBoundyBox(){
		std::vector<double> boundybox{ m_dMinX, m_dMinY, m_dMinZ, m_dMaxX, m_dMaxY, m_dMaxZ };
		return boundybox;
	}
};
